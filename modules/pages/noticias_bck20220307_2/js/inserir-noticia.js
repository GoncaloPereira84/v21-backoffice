...
$('#uploadPhotoModal button#close').on('click', function() {
...
$('#uploadPDFModal button#close').on('click', function() {
    if ($("#uploadPDFModal #files-pdf .valueLine.active").length > 0) {
        ...
        $('#pdf-selected').append('<span>PDF carregado: </span><a id="pdf-id-' + pdf_id + '" href="' + pdf_src + '" target="_blank">' + pdf_src + '</a>');
        // })
    } else {
        $('#pdf-selected').html('');
    }
...
$('#uploadPhotoGalleryModal button#close').on('click', function() {
